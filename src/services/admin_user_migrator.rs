//! Bulk admin-user migration (batches of N to /migration/bulk-add-admin-users).
...
const COMPONENT: &str = "AdminUserMigrator";
...
pub struct AdminUserMigrationResult {
...
impl AdminUserMigrator {
    pub fn new(strapi: StrapiClient) -> Self {
        Self {
            strapi,
            mapping_path: PathBuf::from("data/mappings/users/admin-users.json"),
            failed_path: PathBuf::from("data/reports/admin-users-failed.json"),
            logger: None,
        }
    }
    ...
    pub fn load_admin_users_from_json(path: impl AsRef<Path>) -> Result<Vec<AdminUser>> {
        let raw = fs::read_to_string(path.as_ref())
            .with_context(|| format!("read admin users: {}", path.as_ref().display()))?;
        let users: Vec<AdminUser> = serde_json::from_str(&raw)?;
        Ok(users)
    }

    pub async fn migrate(&self, users: &[AdminUser], image_mapping: &HashMap<String, i64>, batch_size: usize) -> Result<AdminUserMigrationResult> {
        let mut out = AdminUserMigrationResult::default();
        out.total = users.len() as i32;
        let t0 = std::time::Instant::now();

        self.jlog(LogMetadata::new(LogLevel::Info, COMPONENT)
            .with_operation("admin_user_migration_start")
            .with_message(&format!("Migrating {} admin users in batches of {}", users.len(), batch_size)));

        for (i, chunk) in users.chunks(batch_size.max(1)).enumerate() {
            let batch_num = i + 1;
            let arr: Vec<Value> = chunk.iter().map(|u| {
                let email = if u.email.is_empty() {
                    format!("{}@placeholder.com", u.username)
                } else {
                    u.email.clone()
                };
                let mut entry = json!({
            ...
            let mut batch_failures: Vec<AdminBatchFailure> = Vec::new();

            match self.strapi.bulk_add_admin_users(&payload.to_string()).await {
                Ok(resp) if resp.is_success() => {
                    if let Ok(v) = serde_json::from_str::<Value>(&resp.body) {
                        ...
                        // C++ checks both failedAdmins (preferred) and failedUsers (legacy)
                        if let Some(arr) = data.get("failedAdmins").or_else(|| data.get("failedUsers"))
                            .and_then(|x| x.as_array())
                        {
                            for it in arr {
                                if let Some(wp) = it.get("wpId").and_then(|x| x.as_i64()) {
                                    ...
                                    out.failed += 1;
                                    batch_failures.push(AdminBatchFailure { wp_id: wp, reason });
                                }
                            }
                    ...
                    save_mapping(&self.mapping_path, &out.success_map)?;
                    if !batch_failures.is_empty() {
                        save_failed_with_reasons(&self.failed_path, chunk, &batch_failures)?;
                    }
                }
                Ok(resp) => {
                    warn!(batch = batch_num, status = resp.status_code, "admin batch failed");
                    ...
                    for u in chunk {
                        ...
                        out.failed += 1;
                        batch_failures.push(AdminBatchFailure { wp_id: u.wp_id, reason: reason.clone() });
                    }
                    save_failed_with_reasons(&self.failed_path, chunk, &batch_failures)?;
                }
                Err(e) => {
                    warn!(batch = batch_num, error = %e, "admin batch error");
            ...
            info!(batch = batch_num, succeeded = out.succeeded, failed = out.failed, "admin batch done");
            self.jlog(LogMetadata {
                level: Some(LogLevel::Info),
                component: Some(COMPONENT.into()),
                operation: Some("admin_batch_migrated".into()),
        ...
        self.jlog(LogMetadata {
            level: Some(LogLevel::Info),
            component: Some(COMPONENT.into()),
            operation: Some("admin_user_migration_complete".into()),
...
fn save_mapping(path: &Path, map: &HashMap<i64, i64>) -> Result<()> {
...
fn save_failed_with_reasons(
    ...
    let by_id: HashMap<i64, &AdminUser> = chunk.iter().map(|u| (u.wp_id, u)).collect();
